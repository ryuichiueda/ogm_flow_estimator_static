...
pub fn generate(width: u32, height: u32, resolution: f32, scan: &LaserScan) -> OccupancyGrid {
        ...
        let mut map = OccupancyGrid { header, info, data: vec![0; size], };
        scan_to_occupancy(&mut map, scan);

        map
}

pub fn scan_to_occupancy(map: &mut OccupancyGrid, scan: &LaserScan) {
...
pub fn plot(map: &mut OccupancyGrid, x: f64, y: f64, val: i8) {
